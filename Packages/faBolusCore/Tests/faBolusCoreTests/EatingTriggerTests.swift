...
final class EatingTriggerTests: XCTestCase {
    ...
    func testRecentBolusSuppresses() {
        var c = EatingTriggerConfig()
        c.mode = .either
        c.confirmationDelaySeconds = 0
    ...
    func testSignalsMetIgnoresGates() {
        ...
        var c0 = c
        c0.confirmationDelaySeconds = 0
        let e0 = EatingTriggerEngine(config: c0)
        if case .suppress = e0.evaluate(EatingSignals(accelProb: 0.9, minutesSinceBolus: 2), now: t0) {
        } else {
    ...
    func testBothAlwaysRequiresBothSignals() {
        ...
        if case .hold = e.evaluate(EatingSignals(accelProb: 0.9, cgmMealScore: 0.2, minutesSinceBolus: 999), now: t0) {
            ...
            XCTFail("partial should hold")
        }
        XCTAssertEqual(
            e.evaluate(EatingSignals(accelProb: 0.9, cgmMealScore: 0.8, minutesSinceBolus: 999), now: t0), .fire)
    }
    ...
    func testCgmThenAccelRequiresCgmThenAccelConfirm() {
        ...
        if case .hold = e.evaluate(EatingSignals(accelProb: nil, cgmMealScore: 0.8, minutesSinceBolus: 999), now: t0) {
            ...
            XCTFail("CGM alone should hold until wrist confirms")
        }
    ...
    func testEitherFiresOnOneSignal() {
        ...
        let e = EatingTriggerEngine(config: c)
        XCTAssertEqual(
            e.evaluate(EatingSignals(accelProb: 0.9, cgmMealScore: 0.0, minutesSinceBolus: 999), now: t0), .fire)
    }
    ...
    func testEstimatorDirectionAndBattery() {
        var strict = EatingTriggerConfig()
        strict.mode = .cgmThenAccel
        strict.confirmationDelaySeconds = 120
        var loose = EatingTriggerConfig()
        loose.mode = .either
        loose.confirmationDelaySeconds = 0
    ...
    func testCustomAccelMetricsOverrideDefault() {
        var c = EatingTriggerConfig()
        c.mode = .accelOnly
        c.accelThreshold = 0.9
        c.confirmationDelaySeconds = 0
        let m = EatingModelMetrics(
            operatingPoints: [.init(threshold: 0.9, recall: 0.8, falseAlertsPerDay: 1.0)],
            source: "test")
