...
public struct RemoteCommand: Codable, Equatable, Sendable {
    ...
    public var glucoseDisplayUnit: String?
    ...
    public var extendedNowUnits: Double?
    ...
    public var remoteEstimateUnits: Double?
    ...
    public var includeStaleBG: Bool?
    ...
    public var bolusBlockReason: String?
    ...
    public var supportsDismissAck: Bool?
    ...
    public var rawAlerts: [RemoteAlert]?
    ...
    public var bolusPasscode: String?
    ...
    public var controllerVariant: String?
    ...
    public var ciqSuspendStartEpochSec: Int?
    ...
    public var lastAutoCorrectionEpochSec: Int?
    ...
    public var ciqLastCouldNotDeliverEpochSec: Int?
    ...
    /// Additive; auto-Codable, so the existing memberwise initializer stays untouched.
    public var lockoutUntilEpochSec: Int?
    ...
    /// generated by the app, and every watch-facing surface reads this one field, mapping the intent
    /// to its own ladder. Each value is `"off"` (nothing), `"quiet"` (visual only), or `"alert"`
    /// (the vibrating rung); the watch ladder has no breakthrough rung. CONTRACT: an absent field,
    /// an absent key, or an unrecognized value means the receiver fails safe to the vibrating rung —
    /// never silent — so a legacy host that omits it can never quiet a pump safety alert. An explicit
    ...
    /// The wire token for a resolved watch intent. The watch ladder tops out at the vibrating rung
    /// (no breakthrough), so an `.urgent` is capped to `"alert"` here — a receiver never sees a rung
    /// it cannot honor. The one producer for `watchNotificationIntents`; keeping the encode and the
    /// `resolvedWatchIntent(for:)` decode side by side keeps the token vocabulary in one place.
    public static func watchIntentWireToken(_ intent: NotificationRules.Intent) -> String {
        switch intent {
        case .off: return "off"
        case .quiet: return "quiet"
        case .alert, .urgent: return "alert"
        }
    }

    /// The resolved watch intent for `category`, applying the fail-safe contract documented on
    /// `watchNotificationIntents`: an absent field, an absent key, or an unrecognized token all
    /// resolve to the vibrating rung (`.alert`) — never to silence — so an old host or a corrupted
    /// value can never quiet a pump safety alert. A recognized token, including an explicit `"off"`,
    /// is honored as the user's own choice.
    public func resolvedWatchIntent(for category: String) -> NotificationRules.Intent {
        switch watchNotificationIntents?[category] {
        case "off": return .off
        case "quiet": return .quiet
        case "alert": return .alert
        default: return .alert  // absent field / absent key / unrecognized token -> fail safe to vibrate
        }
    }
