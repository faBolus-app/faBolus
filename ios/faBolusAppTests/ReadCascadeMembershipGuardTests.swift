...
struct ReadCascadeMembershipGuardTests {
    ...
    @Test func startPollingDispatchesTheExactOrderedTrioThenFastThenStaticThenAlert() async {
        ...
        #expect(
            dispatched.count == 16,
            "trio (3) + fastRead's 6 non-gated (op20 deferred) + staticRead (7) must be synchronous")
        #expect(Array(dispatched[0..<3]) == Self.bootstrapTrio, "bootstrap trio must be first, in this exact order")
        #expect(
            Array(dispatched[3..<9]) == Self.fastReadTierPreVersion,
            "fastRead's 6 non-gated reads must follow, in this exact order (op20 deferred)")
        #expect(Array(dispatched[9..<16]) == Self.staticReadTier, "staticRead's 7 must follow, in this exact order")
        #expect(
            !dispatched.contains("LoadStatusRequest"),
            "op20 must NOT appear in the pre-version burst — it is identity-gated")
    ...
    @Test func recurringFastAndStaticTickDispatchesExactOrderedFastThenStaticWithNoBootstrap() {
        ...
        #expect(
            dispatched.first != "ApiVersionRequest",
    ...
    @Test func recurringPollTickFiresAlertsEveryTickFastOnMod4StaticOnMod40() async {
        ...
        #expect(
            tick4 == Self.fastReadTier + Self.alertReadTier,
        ...
        for tick in 5...39 {
            ...
            if tick % 4 == 0 {
                #expect(
                    dispatched == Self.fastReadTier + Self.alertReadTier,
        ...
        #expect(
            tick40 == Self.fastReadTier + Self.staticReadTier + Self.alertReadTier,
