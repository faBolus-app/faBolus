...
final class AlertRuleEngineTests: XCTestCase {
    ...
    func testTimeWindowMatch() {
        let rule = AlertRule(
            name: "overnight", kinds: [.cgmAlert],
            startMinuteOfDay: 22 * 60, endMinuteOfDay: 7 * 60, action: .autoSnooze)  // wraps midnight
        XCTAssertEqual(
            AlertRuleEngine.action(for: alert(.cgmAlert), rules: [rule], now: at(3), glucose: 90), .autoSnooze)
    ...
    func testKindFilter() {
        ...
        XCTAssertEqual(
            AlertRuleEngine.action(for: alert(.reminder), rules: [rule], now: at(3), glucose: nil), .autoSnooze)
    }
    ...
    func testAlertIdFilter() {
        ...
        XCTAssertEqual(
            AlertRuleEngine.action(for: alert(.alert, id: 42), rules: [rule], now: at(3), glucose: nil), .autoDismiss)
    }
    ...
    func testGlucoseGateNeedsReading() {
        ...
        XCTAssertEqual(
            AlertRuleEngine.action(for: alert(.cgmAlert), rules: [rule], now: at(12), glucose: 300), .autoSnooze)
    }
    ...
    func testDisabledRuleIgnoredAndFirstMatchWins() {
        ...
        XCTAssertEqual(
            AlertRuleEngine.action(for: alert(.reminder), rules: [off, on], now: at(3), glucose: nil), .autoSnooze)
    }
    ...
    func testFullDayWindowAlwaysMatches() {
        ...
        XCTAssertEqual(
            AlertRuleEngine.action(for: alert(.reminder), rules: [rule], now: at(17, 30), glucose: nil), .autoSnooze)
    }
}
