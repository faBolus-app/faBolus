...
| LoopInsights_CaregiverDigestView.swift | LoopPowerPack/Loop @ ad4c4d4 (© 2026 LoopKit Authors and Taylor Patterson) | MIT | `ios/faBolus/Views/LoopInsights/LoopInsights_CaregiverDigestView.swift` | Re-skinned caregiver-digest surface (period picker + digest preview + share) — a REWRITE of the mirror's CaregiverDigestView, fed by the benign `LoopInsights_CaregiverDigestService` over `FaBolusInsightsAggregator`/`GlucoseHistoryStore`. ALL LoopKit coordinator + MessageUI mail/SMS compose + reminder scheduling + recipient config STRIPPED; the verbatim UI-SPEC PHI disclosure shown before any share; shares via the standard `ShareLink` share sheet. Summary-only §13 copy, no advice/directive/dose (09.18d-03, D-14/D-15/D-17) |
