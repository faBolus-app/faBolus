...
public enum AutoCorrectionDisclosure {
    ...
    ///
    /// Returns `nil` under the exact same guards `lockoutMessage` uses (no controller / controller off at
    /// runtime / documented lockout window unknown), PLUS `nil` when `lockoutStartDate` is absent (no
    /// lockout known) or the window has already elapsed (`elapsed >= windowMinutes` — lockout expired, so
    /// there is no active lockout left to disclose; D-06 guardrail #5 fail-closed). The window length is
    /// read from the descriptor — never restated as a literal here.
