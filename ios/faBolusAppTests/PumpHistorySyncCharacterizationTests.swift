...
struct PumpHistorySyncCharacterizationTests {
    ...
    @Test func retryPagingOrderAcrossTwoWindows() {
        withCleanCoverage {
            ...
            #expect(pages.count == 3, "interior window (1 page) + forward window (2 pages, 255+45) == 3 total page requests")
            if pages.count == 3 {
                ...
                #expect(pages[0].startLog == 400 - (101 - 1), "first page of the FIRST queued window (300...400) walks back from its own top")
                #expect(pages[0].numberOfLogs == 101)
                #expect(pages[1].startLog == 900 - (255 - 1), "first page of the forward window (601...900) walks back from 900")
    ...
    @Test func syncingTransitionsToPausedOnLinkDrop() {
        withCleanCoverage {
            ...
            #expect(backend.historySyncState == .paused,
