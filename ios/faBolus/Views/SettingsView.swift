...
enum StatsCardCopy {
    static let footer =
        "Adds a dashboard card with Time-in-Range, GMI, average, and variability (CV) over the last ~24 hours of readings held in memory."
}
            ...
            if query.isEmpty {
                ...
                if categoryHits.isEmpty && extraHits.isEmpty {
                    ...
                    ForEach(categoryHits) { e in
                        Button {
                            selectedItem = .category(e.category)
                        } label: {
...
enum SettingsIndex {
    struct Entry: Identifiable {
        ...
        func matches(_ q: String) -> Bool {
            ...
            return title.lowercased().contains(s) || keywords.lowercased().contains(s)
                || category.title.lowercased().contains(s)
        }
    }
    static let entries: [Entry] = [
        ...
        .init(
            title: "Recommendation reasoning", keywords: "reasoning iob max safe estimate breakdown", category: .bolus),
        ...
        .init(
            title: "Dashboard pills", keywords: "reorder hide pills iob reservoir carb isf target", category: .display),
        ...
        .init(
            title: "Allow bolusing from Garmin",
            keywords: "allow enable remote bolus garmin deliver read only view only", category: .remotes),
        .init(
            title: "Remote bolus size limit", keywords: "ceiling cap max units remote bolus limit dose garmin",
            category: .remotes),
        ...
        .init(
            ...
            keywords: "pump app critical breakthrough quiet hours per category mute silence", category: .notifications),
        ...
        .init(
            title: "Garmin complications", keywords: "watch face slots iob reservoir battery basal pump status",
            category: .remotes),
        .init(
            title: "Garmin alert intensity",
            keywords: "vibrate silent audible tone backlight do not disturb dnd critical alarm sound",
            category: .remotes),
...
enum SettingsExtraIndex {
    ...
    static let entries: [Entry] = [
        .init(
            ...
            keywords: "safe viewer caregiver backup phone bolusing disabled pump control hidden clearing alerts",
            item: .safety),
    ...
    var body: some View {
        Form {
            Section {
                ...
                Text(
                    "Default entry mode. **Phone** covers the iPhone and the widget; **Garmin** is independent, for the Garmin bolus screen (the Apple Watch remote is removed)."
                )
            }
            Section {
            ...
            Section {
                ...
                Text(
                    "A collapsible breakdown (IOB, carb + correction, an advisory max-safe estimate) under the recommended dose."
                )
            }
        }
    ...
    var body: some View {
        Form {
            ...
            Section {
                NavigationLink {
                    CustomizeListView(
                        title: "Details", allIds: AppSettings.detailFields,
                    ...
                    LabeledContent("Phone details rows", value: "\(settings.detailsOrder.count) shown")
                }
                NavigationLink {
                    CustomizeListView(
                        title: "Pills", allIds: AppSettings.pillItems,
                    ...
                    LabeledContent("Dashboard pills", value: "\(settings.pillsOrder.count) shown")
                }
            } header: {
                Text("Customize")
            } footer: {
                Text(
                    "Choose which detail rows and pills appear on the phone dashboard. (Garmin details + chart ranges are under Remotes & devices.)"
                )
            }
        }
...
struct CgmSettingsView: View {
    ...
    var body: some View {
        Form {
            Section {
                ...
                Text(
                    "An independent CGM feed used when the pump's glucose goes stale (pump, phone, or sensor link dropped). Old readings are shown marked, never as current. Takes effect after you reopen the app."
                )
            }
            Section {
                NavigationLink {
                    CgmCredentialsView(model: model)
                } label: {
                    VStack(alignment: .leading, spacing: 2) {
            ...
            Section {
                ...
                Text(
                    "**Mark stale**: after this long, a reading is greyed and no longer auto-fills a correction. **Hide**: how long after that to keep showing the greyed value before it becomes “–”."
                )
            }
        }
    ...
    private var currentSelectionSubtitle: (text: String, isActive: Bool) {
        ...
        return CgmStatusView.selectionStatusSubtitle(
            selected: selected,
...
struct PumpSettingsView: View {
    ...
    private enum UnpairStep: Identifiable {
        ...
        var id: String {
            switch self {
            case .confirm(let r): return "confirm-\(r)"
            }
        }
        var repairAfter: Bool {
            switch self {
            case .confirm(let r): return r
            }
        }
    }
    var body: some View {
        ...
        .confirmationDialog(
            "Forget pairing?",
            isPresented: Binding(
                get: { if case .confirm = unpairStep { return true } else { return false } },
...
struct RemotesSettingsView: View {
    ...
    private var remoteCeilingOn: Binding<Bool> {
        Binding(
            get: { settings.remoteBolusCeiling != nil },
            set: { on in
                settings.remoteBolusCeiling =
                    on ? (settings.remoteBolusCeiling ?? AppSettings.defaultRemoteBolusCeiling) : nil
            })
    }
    private var remoteCeilingValue: Binding<Double> {
        Binding(
            get: { settings.remoteBolusCeiling ?? AppSettings.defaultRemoteBolusCeiling },
    ...
    private var smallPlotFloorValue: Binding<Int> {
        Binding(
            get: { settings.glucosePlotFloorSmall ?? settings.glucosePlotFloor },
            set: { settings.glucosePlotFloorSmall = $0 })
    }
    private var smallPlotCeilingValue: Binding<Int> {
        Binding(
            get: { settings.glucosePlotCeilingSmall ?? settings.glucosePlotCeiling },
    ...
    var body: some View {
        Form {
            ...
            Section {
                ...
                NavigationLink {
                    ...
                    LabeledContent(
                        "Screen order",
                        value: AppSettings.garminScreenLabel(settings.garminDefaultScreen).components(separatedBy: " (")
                            .first ?? settings.garminDefaultScreen)
                }
                Picker("Complication display", selection: $settings.garminComplicationDisplay) {
                    ForEach(AppSettings.complicationDisplayOptions, id: \.self) {
                        Text(AppSettings.complicationDisplayLabel($0)).tag($0)
                    }
                }
                ...
                Text(
                    "Reorder the Garmin app's swipe screens, and choose how the watch-face BG complication looks. Applied on the watch's next update. ⚠️ If the complication doesn't show correctly, switch the display mode — the color path uses a complication field that's unverified on-device (see docs/UNVERIFIED-GUESSES.md)."
                )
            }
            #else
            Section {
                ...
                Text(
                    "This build was compiled **without the Garmin Connect IQ SDK**, so the Garmin remote can't be enabled. To use a Garmin watch, install the Connect IQ SDK and rebuild (see the README / scripts/generate-project.sh)."
                )
            }
            #endif
            #if !WATCH_EMBEDDED
            Section {
                ...
                Text(
                    "faBolus no longer includes an Apple Watch app — nothing installs to a paired watch. Use the Garmin remote below, or the iPhone app directly."
                )
            }
            #endif
            Section {
                ...
                Text(
                    "Default entry mode and increments for the Garmin bolus screen (independent of the iPhone; the Apple Watch remote is removed). Same settings as under Bolus & entry."
                )
            }
            Section {
                NavigationLink {
                    CustomizeListView(
                        title: "Garmin details", allIds: AppSettings.detailFields,
                        label: AppSettings.detailFieldLabel, order: $settings.watchDetailsOrder,
                        shownFooter:
                            "Rows shown on the Garmin Details page (independent of the phone). Drag to reorder, swipe to hide."
                    )
                } label: {
                    LabeledContent("Garmin details rows", value: "\(settings.watchDetailsOrder.count) shown")
                }
                NavigationLink {
                    WatchChartRangesView(settings: settings)
                } label: {
                    LabeledContent(
                        "Garmin chart ranges", value: settings.watchChartRanges.map { "\($0)h" }.joined(separator: " "))
                }
                ...
                NavigationLink {
                    GarminComplicationsView(settings: settings)
                } label: {
                    LabeledContent(
                        "Garmin complications",
                        value: settings.garminComplicationSlots.isEmpty
                            ? "None"
                            : settings.garminComplicationSlots.map {
                                AppSettings.garminComplicationFieldLabel($0).components(separatedBy: " (").first ?? $0
                            }.joined(separator: ", "))
                }
            } header: {
                Text("Garmin display")
        ...
        .confirmationDialog(
            "Allow bolusing from Garmin?", isPresented: $showGarminBolusWarning,
            titleVisibility: .visible
        ) {
            Button("Allow bolusing") {
                settings.acknowledgeGarminBolusWarning()
                settings.garminBolusEnabled = true
            }
            Button("Cancel", role: .cancel) {}
        } message: {
            Text(
                "This lets you deliver real insulin from your Garmin watch. A Garmin has no wrist detection, so take extra care that a bolus is never started by an accidental button press — it stays off until you allow it here, and every bolus still needs your confirmation on the watch. You can turn this off any time."
            )
        }
    }
}
...
struct BolusPasscodeEntryView: View {
            ...
            Form {
                Section {
                    ...
                    if let error {
                        ...
                        Text(
                            "Choose a 4-digit passcode. You'll enter it on your Garmin watch to confirm a bolus. It's stored only as a salted hash on this phone — never sent to the watch."
                        )
                    }
                }
            }
    ...
    private func submit() {
        ...
        guard BolusPasscodeStore.isValidFormat(digits) else {
        ...
        guard onSet(digits) else {
            error = "Couldn't save the passcode. Try again."
            return
        }
        dismiss()
    }
    ...
    var body: some View {
        Form {
            ...
            Section {
                ForEach(settings.garminScreenOrder, id: \.self) { id in
                    Label(
                        AppSettings.garminScreenLabel(id),
                ...
                Text(
                    "Swiping up on the watch moves down this list. Swipe a row left to hide it — at least one screen must stay shown."
                )
            }
    ...
    var body: some View {
        Form {
            Section {
                ...
                Text(
                    "Tapping the Garmin history chart cycles through the enabled ranges. At least one must stay enabled."
                )
            }
        }
