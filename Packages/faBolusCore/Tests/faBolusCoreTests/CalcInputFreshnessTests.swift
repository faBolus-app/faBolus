...
final class CalcInputFreshnessTests: XCTestCase {
    ...
    func testFutureDatedIsStaleBeyondClockSkew() {
        ...
        XCTAssertTrue(
            CalcInputFreshness.isIobStale(now.addingTimeInterval(CalcInputFreshness.futureSkewTolerance + 1), now: now))
        XCTAssertFalse(
            CalcInputFreshness.isIobStale(now.addingTimeInterval(CalcInputFreshness.futureSkewTolerance - 1), now: now))
    }
