...
/// Local-first, OPT-IN connection telemetry (connection uptime, disconnect reasons,
/// reconciliation outcomes). Shares the App-Group container AND the SAME opt-in flag
/// (`NotificationRuntime.telemetryEnabledKey`), so one "share local diagnostics" choice governs
/// both; default OFF, never uploaded. Read-modify-write persisted so a sibling process can't clobber it.
@MainActor
final class ConnectionTelemetryStore {
    private let store: UserDefaults
    private let key = AppGroupKeys.connectionTelemetry
    ...
    /// The shared opt-in — the same flag notification telemetry uses (one diagnostics switch).
    ...
    /// Erase the accumulated diagnostics blob (for "Delete all on-device data"). Leaves the
    ...
    /// A command round-trip completed. `seconds != nil` → count its latency bucket;
    ...
    /// app boundary), so no new backend/protocol plumbing is needed. `nil` detail is a
    /// plain drop or a user-initiated disconnect ("dropped"); the known radio-down phrases map to their
    /// own tokens. `TandemBackend.applyClientError` populates `connectionDetail` with a
    ...
    ///
    /// For `CBErrorDomain` specifically, append the human-readable label from `cbErrorLabels` when
    /// the extracted code is a known key (0–18) — e.g. "CBErrorDomain#7 → Peripheral disconnected".
    /// Any other domain, or a code outside 0–18, falls through unchanged to the raw `domain#code`
    /// token (fail-closed) — never crashes, never emits unbounded text.
    ...
    /// `CBError.Code` raw-value table (0–18), cross-checked against this project's own on-device
    /// `CBErrorDomain#7` capture — matches `peripheralDisconnected`. Additive only:
    /// `domainCodeToken(from:)` falls back to the raw token for any code not present here.
