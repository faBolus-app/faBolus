...
public enum NotificationBroker {
    ...
    public enum Category: String, CaseIterable, Sendable, Codable {
        ...
        /// A pure display/classification axis (D-02, Phase 8.1) driving the settings UI's pump-vs-app
        /// two-section split. `pumpAlert` is the only category relayed FROM the pump; every other category
        /// (incl. all three never-suppressible trio categories) is GENERATED BY the app. This is display-
        /// only — `decide()` never reads it, so it cannot influence governance.
