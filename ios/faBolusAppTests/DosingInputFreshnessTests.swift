...
struct DosingInputFreshnessTests {
    ...
    private func makeModel(connected: Bool) async -> (AppModel, MockBackend, AppModelBehaviorTests.EchoRecorder) {
        ...
        let rec = AppModelBehaviorTests.EchoRecorder(); rec.attach(to: model)
    ...
    private func withCleanGates(_ body: () async -> Void) async {
        ...
        s.phoneReadOnly = false; s.childModeEnabled = false; s.remotesReadOnly = false; s.appMode = .advanced
        defer { s.phoneReadOnly = ro; s.childModeEnabled = child; s.remotesReadOnly = rro; s.appMode = mode }
        await body()
    }
    ...
    @Test func divergenceGuardFiresWhenAnInputChangesBetweenComposeAndDeliver() async {
        await withCleanGates {
            ...
            await model.remoteDeliver(requestId: "dif-div", carbsGrams: 0, remoteEstimate: composeEstimate, peerId: "watch")
