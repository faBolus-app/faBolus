...
struct GlucoseWidgetView: View {
    ...
    /// Same fresh/stale/hidden logic as `WidgetUI.glucoseText(_:now:)`, but the
    ...
    /// plus the `g > 0` guard `bg` already applies).
    ...
    /// speaks the band word for a live reading so VoiceOver never depends on zone color alone.
    ...
    var body: some View {
        ...
        case .accessoryInline:
            ...
            // this is a neutral, non-status icon, not a good/bad band glyph.
        ...
        case .accessoryCircular:
            ZStack {
                ...
                // Combine the value+arrow into one spoken element and add the band word
                // (VoiceOver must not depend on zone color alone).
        ...
        default:  // .systemSmall
            VStack(alignment: .leading, spacing: 2) {
                ...
                // Combine the value+arrow row into one spoken element carrying the band word;
                ...
                HStack {
                    ...
                    // current. Live-updating relative text keyed off the entry date.
