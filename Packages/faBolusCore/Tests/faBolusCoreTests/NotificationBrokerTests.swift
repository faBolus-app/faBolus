...
@Suite struct NotificationBrokerTests {
    ...
    @Test func forceProtectedSafetyClassesAreNeverAutoSuppressed() {
        ...
        #expect(
            B.autoSuppression(for: cgm, safetyClass: .cgmDataLoss, rules: [rule], now: now, calendar: cal, glucose: nil)
                == nil)
        // Low-insulin (kind .alert) and occlusion are force-protected too.
        let low = PumpAlert(id: 0, kind: .alert, title: "Low insulin")
        #expect(
            B.autoSuppression(for: low, safetyClass: .lowInsulin, rules: [rule], now: now, calendar: cal, glucose: nil)
                == nil)
        let occ = PumpAlert(id: 2, kind: .alarm, title: "Occlusion")
        #expect(
            B.autoSuppression(for: occ, safetyClass: .occlusion, rules: [rule], now: now, calendar: cal, glucose: nil)
                == nil)
        ...
        #expect(
            B.autoSuppression(for: other, safetyClass: .other, rules: [rule], now: now, calendar: cal, glucose: nil)
                == .autoSnooze)
    }
    ...
    @Test func semanticClassNotPumpKindDrivesProtection() {
        ...
        #expect(
            B.autoSuppression(for: low, safetyClass: .lowInsulin, rules: [rule], now: now, calendar: cal, glucose: nil)
                == nil)
        #expect(
            B.autoSuppression(for: plain, safetyClass: .other, rules: [rule], now: now, calendar: cal, glucose: nil)
                == .autoSnooze)
