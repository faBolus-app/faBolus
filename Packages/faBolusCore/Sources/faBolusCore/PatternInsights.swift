...
public final class PatternInsights {
    ...
    public func insights(cgm: [CGMPoint], carbs: [Carbs] = []) -> [Insight] {
        ...
        if let (h, frac) = (0..<24).compactMap({ n[$0] > 20 ? ($0, Double(lowN[$0])/Double(n[$0])) : nil })
            ...
            out.append(.init(title: "Recurring lows around \(hourLabel(h))",
                             detail: "About \(Int(frac*100))% of readings near \(hourLabel(h)) are below \(Int(cfg.low)) mg/dL. Consider less insulin / a snack before then, or discuss basal timing.",
                             severity: 2))
        }
        ...
        if let (h, frac) = (0..<24).compactMap({ n[$0] > 20 ? ($0, Double(highN[$0])/Double(n[$0])) : nil })
            ...
            out.append(.init(title: "Highs concentrated around \(hourLabel(h))",
                             detail: "Readings near \(hourLabel(h)) are above \(Int(cfg.high)) mg/dL ~\(Int(frac*100))% of the time.",
                             severity: 1))
        }
        ...
        if dawnRises.count >= 3, dawnRises.filter({ $0 > 30 }).count >= Int(0.5*Double(dawnRises.count)) {
            ...
            out.append(.init(title: "Dawn rise most mornings",
                             detail: "Glucose rises on average \(Int(avg)) mg/dL between 3–7am. A basal adjustment before dawn may help — discuss with your clinician.",
                             severity: 1))
        }
        // overall TIR
        ...
        out.append(.init(title: "Time in range: \(Int(tir*100))%",
                         detail: "Over the last \(Int(span)) days, \(Int(tir*100))% of readings were \(Int(cfg.low))–\(Int(cfg.high)) mg/dL.",
