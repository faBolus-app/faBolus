...
struct RefreshOrderingCharacterizationTests {
    ...
    private static let spine: [String] = [
        ...
        "updateEatingNudge", "reconcileHeartRateWanted", "evaluateSavePinOffer", "maybeAutoSyncPumpTime",
        "statusPush",
    ]
    ...
    @Test func topLevelOrderIsSwitchThenMergeThenFacadeAssignThenEffects() async {
        ...
        #expect(Array(recorded.prefix(3)) == ["maybeHandlePumpSwitch", "merge", "facadeAssign"],
        ...
        #expect(spineProjection(of: recorded) == Self.spine,
        ...
        for (i, tag) in recorded.enumerated() where i > facadeIdx {
            #expect(tag != "maybeHandlePumpSwitch" && tag != "merge",
    ...
    @Test func pumpSwitchResetsTherapyParamsBeforeMergeReadsThem() async {
        ...
        #expect(backend.snapshot.carbRatio == 10 && backend.snapshot.isf == 40 && backend.snapshot.targetBg == 110,
    ...
    @Test func safetyEdgesAreComputedFromPreAssignmentState() async {
        ...
        #expect(recorded.contains("freshnessEdge:raise"),
                "fresh→stale must RAISE — computed from the pre-assignment previousGlucoseFresh == true")
        #expect(recorded.contains("connectionEdge:none"),
        ...
        #expect(recorded.contains("connectionEdge:raise"),
