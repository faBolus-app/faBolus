...
A Loop-style remote at parity with the phone and Garmin. **The watch never touches the pump** —
...
complication (long-press the face → **Edit** → pick a corner → **ControlX2** → Glucose). It reads
...
!!! note "One-time setup (App Group)"
    ...
    `group.com.zgranowitz.controlx2`. It's enabled once on the **ControlX2Watch** and
    **ControlX2WatchWidgets** targets (Xcode → each target → Signing & Capabilities → App Groups),
